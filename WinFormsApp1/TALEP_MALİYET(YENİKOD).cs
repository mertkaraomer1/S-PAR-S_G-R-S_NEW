...
{
    public partial class TALEP_MALİYET_YENİKOD_ : Form
    {
        private MyDbContext dbContext;
        public TALEP_MALİYET_YENİKOD_()
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                double fiyat = 0;
                var tuketimkod = dbContext.URUN_RECETELERI.Where(x => x.rec_anakod.Substring(0, 13) == item.pro_stokkodu && !x.rec_tuketim_kod.StartsWith("9"))
                 .Select(x => new
                 {
                ...
                var tuketimkod1 = dbContext.URUN_RECETELERI.Where(x => x.rec_anakod.Substring(0, 13) == item.pro_stokkodu && x.rec_tuketim_kod.StartsWith("9"))
                     .Select(x => new
                     {
                         item.pro_evrakno_sira,
                         item.pro_stokkodu,
                   ...
                   .FirstOrDefault(); // İlk grup için öğeyi seç
                    ...
                    var İşçiMaliyetiTamamlanan = dbContext.URETIM_OPERASYON_HAREKETLERI
                        ...
                        .Select(x => new
                        {
                            ...
                            TopFiyatIsciTam = x.Sum(y =>
                                (y.OpT_TamamlananMiktar > 0 ? (y.OpT_TamamlananSure / y.OpT_TamamlananMiktar) * item.pro_miktar : 0)) / 60 * DKIsciMaliyet,
                        })
                        .FirstOrDefault();
                ...
                var MalzemeSatisFiyati1 = dbContext.SIPARISLER
                      .Where(x => x.sip_stok_kod.Length > 13 && x.sip_stok_kod.Substring(0, 13) == item.pro_stokkodu)
                      .OrderByDescending(x => x.sip_create_date)
                      .Select(x => new
                      {
                          x.sip_stok_kod,
                          x.sip_tarih,
                          TopFiyatSatis = x.sip_b_fiyat * x.sip_doviz_kuru * item.pro_miktar,
                      })
                      .FirstOrDefault();
        ...
        private void TALEP_MALİYET_YENİKOD__Load(object sender, EventArgs e)
        {
